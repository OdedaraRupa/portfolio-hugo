...
(function ($) {
  ...
  function Headline($el) {
    if ($.isFunction($.fn.animatedHeadline)) {
      $el.find('.ah-headline:not(.pixel-active-line)').animatedHeadline();
      $el.find('.ah-headline').addClass('pixel-active-line');
    }
  }

  function accordion($el = $(document)) {
    $el.find(".pixel-accordion").each(function () {
      const $this = $(this),
  ...
  function initMap($el = $(document)) {
    ...
    if (mapScriptId === null) {
      const GOOGLE_MAP_KEY = pixelParam.map.api,
    ...
    setTimeout(function () {
      try {
        map_id.each(function () {
          let map_att = $(this),
              lat = pixelGrid.getData(this, "lat"),
              leg = pixelGrid.getData(this, "len"),
              zoom = pixelGrid.getData(this, "zoom");
          ...
          new google.maps.Marker({
            ...
            icon: pixelParam.map.marker_icon,
            title: "ASL",
            map: map
          });
        });
  ...
  function justifiedGallery($el = $(document)) {
    $el.find(".gallery-portfolio").each(function () {
      ...
      const option = pixelGrid.getData(this, 'option') || {};
  ...
  const activeNav = function ($index) {
    ...
    const $nav = $(this).find('.pixel-nex-prev');
    const prev = $nav.find('.prev .pixel-wrapper .pixel-slide');
    const next = $nav.find('.next .pixel-wrapper .pixel-slide');
    prev.removeClass('pixel-active');
    next.removeClass('pixel-active');
    (_prev$get = prev.get($index)) === null || _prev$get === void 0 ? void 0 : _prev$get.classList.add('pixel-active');
    (_next$get = next.get($index)) === null || _next$get === void 0 ? void 0 : _next$get.classList.add('pixel-active');
  };
  ...
  function sliderPortfolio($el = $(document)) {
    ...
    const initSlider = async function () {
      const pixelSliderContent = this.querySelector('.pixel-slider-content');
      await this.querySelectorAll('.content-slider .slide-item').forEach(($item, $index) => {
        ...
        $item.setAttribute('data-pixel-id', $index);
        if (!slideContent) return;
        if (!slideContent) return;
        slideContent === null || slideContent === void 0 ? void 0 : slideContent.setAttribute('data-pixel-id', $index);
        if ($index === 0) slideContent === null || slideContent === void 0 ? void 0 : slideContent.classList.add('pixel-active', 'pixel-active-animate');
        /**
         *
         * @type {*|jQuery}
         */
        ...
        const pushItem = `<div class="pixel-slide d-flex align-items-center justify-content-center">${img.prop('outerHTML')}<h6 class="sm-title-block">${(_slideContent$querySe = slideContent.querySelector('.title-lg')) === null || _slideContent$querySe === void 0 ? void 0 : _slideContent$querySe.innerText}</h6></div>`;
        pagination.prev.push(pushItem);
        pagination.next.push(pushItem);
        pixelSliderContent === null || pixelSliderContent === void 0 ? void 0 : pixelSliderContent.append(slideContent);
      });
      pagination.next.push(pagination.next.shift());
      pagination.prev.unshift(pagination.prev.pop());
      $(this).find('.pixel-nex-prev .prev .pixel-wrapper').html(pagination.prev.join(''));
      $(this).find('.pixel-nex-prev .next .pixel-wrapper').html(pagination.next.join(''));
      activeNav.bind(this)(0);
      await this.querySelectorAll('.pixel-slider-content .slide-content [data-pixel-split=\"chars\"]').forEach(function ($item) {
        pixelGrid.spltting.Char($item, true);
      }); // await this.querySelectorAll('.pixel-slider-content .head-meta , .pixel-slider-content p.description , .pixel-slider-content .pixel-btn').forEach(function ($item) {
      //     pixelGrid.spltting.Items($item, "span");
      // });

      await this.querySelectorAll('.pixel-slider-content .slide-content').forEach(function ($item) {
        pixelGrid.spltting.Items($item, ".head-meta span , p.description , .pixel-btn");
      });
    };
    ...
    const getContent = (swiper, contentRef) => {
      const oldNum = swiper.slides[swiper.previousIndex].getAttribute("data-pixel-id");
      const newNum = swiper.slides[swiper.activeIndex].getAttribute("data-pixel-id");
      return [newNum, oldNum, contentRef.querySelector('[data-pixel-id="' + newNum + '"]'), contentRef.querySelector('[data-pixel-id="' + oldNum + '"]')];
    };
    ...
    function slideChange(swiper) {
      const contentSlider = this.querySelector('.pixel-slider-content');
      ...
      if (swiper.pixelOnChange) swiper.pixelOnChange(newNum, oldNum, newContent, oldContent);
      tl.progress(1);
      tl = new gsap.timeline();
      oldContent.classList.remove("pixel-active-animate");
      tl.fromTo(oldTitle, 0.4, animate.show, animate.hide).call(function () {
        oldContent.classList.remove("pixel-active");
        newContent.classList.add("pixel-active");
        newContent.classList.add("pixel-active-animate");
    ...
    const NavSwiper = function (container, swiper) {
      ...
      const navContainer = container.querySelector('.pixel-nex-prev .next .swiper');
      if (!navContainer) return false;

      if (window.innerWidth > 575) {
        navContainer.querySelectorAll('h6.sm-title-block').forEach($item => {
          const s = pixelGrid.spltting.Char($item);
    ...
    const NavSwiperPrev = function (container, swiper, nav) {
      ...
      const navContainer = container.querySelector('.pixel-nex-prev .prev .swiper');
      if (!navContainer) return false;
    ...
    $el.find(".main-slider:not(.pixel-swiper-initialized)").each(function () {
      this.classList.add('pixel-swiper-initialized');
      initSlider.bind(this)().then(swiper.bind(this)).then(function (swiper) {
        ...
        const elementNextPrev = $(this).find('.pixel-nex-prev .prev , .pixel-nex-prev .next');

        const HoverNextPrev = function (e) {
          elementNextPrev.removeClass('pixel-active');
          $(this).addClass('pixel-active');
        };

        $this.on('click', '.pixel-nex-prev .next', handleNext);
        $this.on('click', '.pixel-nex-prev .prev', handlePrev);
        $this.on('mouseenter', '.pixel-nex-prev .prev , .pixel-nex-prev .next', HoverNextPrev);
        let webGel = null;
        if (this.classList.contains('pixel-webgl')) webGel = pixelGrid.WebGLDistortionHoverEffects({
          imgs: webGelOption.bind(this)(),
          ...(pixelGrid.getData(this, 'webgl', {}) || {}),
        ...
        pixelGrid.killAjax(function () {
          $this.off('click', '.pixel-nex-prev .next', handleNext);
          $this.off('click', '.pixel-nex-prev .prev', handlePrev);
          tl.kill();
          swiper.destroy();
          webGel.destroy();
        });
